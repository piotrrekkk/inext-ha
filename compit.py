"""
Python wrapper for getting interaction with inext devices.
"""
import logging
import aiohttp
import json
...
class Compit:
    """Main class to perform inext API requests"""

    COMPIT_API_URL = "https://inext.compit.pl/api/"
    COMPIT_LOGIN_URL = "https://inext.compit.pl/pl/login"
    def __init__(self, session: aiohttp.ClientSession, user_id = None, token = None, base_url = COMPIT_API_URL, update_interval = 30):
        _LOGGER.debug("Init iNext")
        self.headers = {
    ...
    async def get(self, request_path):
        request_path = COMPIT_API_URL
        _LOGGER.debug("Sending GET request: " + request_path)
        async with self.session.get(request_path, headers=self.headers) as response:
...
async def put(self, request_path):
        request_path = COMPIT_API_URL
        _LOGGER.debug("Sending PUT request: " + COMPIT_API_URL)
        async with self.session.get(request_path, headers=self.headers) as response:
            if response.status != 200:
                _LOGGER.warning("Invalid response from Tech API: %s", response.status)
                raise TechError(response.status, await response.text())
    ...
    async def post(self, request_path, post_data):
        _LOGGER.debug("Sending POST request: " + request_path)
        async with self.session.post(request_path, data=post_data, headers=self.headers) as response:
            if response.status != 200:
                _LOGGER.warning("Invalid response from Tech API: %s", response.status)
                raise TechError(response.status, await response.text())
